...
const BlogPosts = () => {
  const { posts } = useProjectsAndPosts();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-12 text-left">
        {POSTS.map(({ slug, background }) => ({
          ...
          <a
            href={slug}
            key={slug}
            className="no-underline text-white relative rounded-2xl overflow-hidden after:border after:border-solid after:border-white/10 after:pointer-events-none after:content-[''] after:absolute after:inset-px after:rounded-2xl px-0 py-0 gb-[#2d0b2d] flex flex-col items-start z-10 flex-1"
          >
            <Img
            ...
            >
              ...
              <div className="text-base font-recursive-mono mb-2">{date}</div>
